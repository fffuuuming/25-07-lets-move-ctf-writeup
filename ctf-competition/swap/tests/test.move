...
module movectf::test {
    ...
    use movectf::token1::{init_for_testing as init_for_testing1, TOKEN1 as T_TOKEN1};
    use movectf::token2::{init_for_testing as init_for_testing2, TOKEN2 as T_TOKEN2};
    use movectf::token3::{init_for_testing as init_for_testing3, TOKEN3 as T_TOKEN3};
    use movectf::token4::{init_for_testing as init_for_testing4, TOKEN4 as T_TOKEN4};
    use movectf::pool::{
        init_for_testing, 
        Pools, AdminCap, 
        init_pools, 
    ...
    fun solve() {
        ...
        {
            init_for_testing1(test_scenario::ctx(scenario));
            init_for_testing2(test_scenario::ctx(scenario));
            init_for_testing3(test_scenario::ctx(scenario));
            init_for_testing4(test_scenario::ctx(scenario));
        ...
        {
            let mut pools = test_scenario::take_shared<Pools>(scenario);
            print(get_pool<T_TOKEN1, TOKEN2>(&mut pools));
            ...
            print(&get_token<T_TOKEN1>(&mut pools).value()) ;
            print(&get_token<T_TOKEN2>(&mut pools).value()) ;
            test_scenario::return_shared(pools);
        };
        ...
        test_scenario::next_tx(scenario, user);
        {
            ...
            print(&get_token<T_TOKEN3>(&mut pools).value()) ;
            print(&get_token<T_TOKEN4>(&mut pools).value()) ;
            test_scenario::return_shared(pools);
        };
        ...
        // let f1 = mint_for_testing<Fake>(100, test_scenario::ctx(scenario));
